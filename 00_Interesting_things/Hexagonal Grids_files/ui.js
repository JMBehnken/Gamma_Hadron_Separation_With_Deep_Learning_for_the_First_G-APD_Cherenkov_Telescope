...
function distanceToScreen(node) {
    ...
    // offsetTop/offsetHeight so I check the parent instead ; also
    // Webkit will lose this in 2016. TODO:
    // https://www.chromestatus.com/features/5724912467574784 says to
...
function updateOffsetConversionCode(style) {
    var code = d3.selectAll("#conversion-offset span").data(updateOffsetConversionCode.styles);
    code.style('display', function(d) { return (d == style)? "inline" : "none"; });
}
updateOffsetConversionCode.styles = ["oddR", "evenR", "oddQ", "evenQ"];
updateOffsetConversionCode.styles.forEach(function(style) {
    d3.select("#conversion-" + style).on('change', function() { updateOffsetConversionCode(style); });
});
updateOffsetConversionCode("oddR");
d3.select("#conversion-oddR").node().checked = true;
...
var neighbors_offset = makeNeighbors("#neighbors-offset-diagram", "#neighbors-offset-code", Grid.cubeToOddR, "row").update(65, true);
function updateNeighborsOffset(label, converter, pointy) {
    return function() {
        d3.selectAll(".neighbors-offset-name").text(label);
        neighbors_offset.converter = converter;
        neighbors_offset.parity_var = pointy? "row" : "col";
        neighbors_offset.update(65, pointy);
    };
}
d3.select("#neighbors-offset-odd-r").on('change', updateNeighborsOffset('oddr', Grid.cubeToOddR, true));
d3.select("#neighbors-offset-even-r").on('change', updateNeighborsOffset('evenr', Grid.cubeToEvenR, true));
d3.select("#neighbors-offset-odd-q").on('change', updateNeighborsOffset('oddq', Grid.cubeToOddQ, false));
d3.select("#neighbors-offset-even-q").on('change', updateNeighborsOffset('evenq', Grid.cubeToEvenQ, false));
updateNeighborsOffset('oddr', Grid.cubeToOddR, true)();
...
function makeLineDrawer() {
    ...
    function redraw() {
        ...
        // FractionalCube.cubeLinedraw. TODO: investigate whether I
        // need this hack at all. I can't produce any examples where
        // it makes a difference, even though in theory it could.
...
function updateHexToPixelOffset(style) {
    d3.select("#hex-to-pixel-code-offset-name").text(style.toLowerCase());
...
var neighbors_cube = makeNeighbors("#neighbors-cube-diagram", "#neighbors-cube-code");
var neighbors_axial = makeNeighbors("#neighbors-axial-diagram", "#neighbors-axial-code", Grid.cubeToTwoAxis, "");

var neighbors_diagonal = makeGridDiagram(d3.select("#neighbors-diagonal-diagram"),
