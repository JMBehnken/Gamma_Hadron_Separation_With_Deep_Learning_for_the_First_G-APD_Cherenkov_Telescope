...
for num_steps, learning_rate, batch_size, patch_size, depth, num_hidden in hyperparameter:
    try:
        ...
        # Fifth layer is a convolution layer
        with tf.name_scope('conv2d_5'):
            layer5_weights = tf.Variable(tf.truncated_normal([patch_size, patch_size, 8*depth, 16*depth], stddev=0.1), name='W')
            layer5_biases = tf.Variable(tf.constant(1.0, shape=[16*depth]), name='B')
        ...
        with tf.name_scope('fc_1'):
            layer6_weights = tf.Variable(tf.truncated_normal([2*2*16*depth, num_hidden], stddev=0.1), name='W')
        ...
        with tf.name_scope('fc_2'):
            layer7_weights = tf.Variable(tf.truncated_normal([num_hidden, num_hidden], stddev=0.1), name='W')
            layer7_biases = tf.Variable(tf.constant(1.0, shape=[num_hidden]), name='B')
    
            hidden = tf.nn.relu(tf.matmul(hidden, layer7_weights) + layer7_biases)
    
            #tf.summary.histogram("weights", layer7_weights)
            #tf.summary.histogram("biases", layer7_biases)
            #tf.summary.histogram("activations", output)
    
        # Eigth layer is a dense output layer
        with tf.name_scope('fc_3'):
            layer8_weights = tf.Variable(tf.truncated_normal([num_hidden, num_labels], stddev=0.1), name='W')
            layer8_biases = tf.Variable(tf.constant(1.0, shape=[num_labels]), name='B')
        ...
        with tf.name_scope('validation'):
            ...
            hidden = tf.nn.relu(tf.matmul(reshape, layer6_weights) + layer6_biases)
            hidden = tf.nn.relu(tf.matmul(hidden, layer7_weights) + layer7_biases)
            valid_prediction = tf.nn.softmax(tf.matmul(hidden, layer8_weights) + layer8_biases)
