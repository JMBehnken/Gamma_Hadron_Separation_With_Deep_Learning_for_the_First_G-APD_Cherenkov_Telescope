...
model_name = 'cccfff'
title_name = 'Plotting_cccdfdfdf'

file_paths = os.listdir(save_model_path)
for path in file_paths:
    name = '_' + model_name
    if path.endswith(name) and 'pre' not in path:
        correct_path = path 
...
num_steps = [500001] * number_of_nets
...
for num_steps, learning_rate, batch_size, patch_size, depth, num_hidden in hyperparameter:
    try:
        ...
        with tf.name_scope('conv2d_3'):
            ...
            pool = tf.nn.max_pool(hidden, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
            pool = tf.nn.dropout(pool, dropout_rate_input)
        ...
        with tf.name_scope('fc_1'):
            layer4_weights = tf.Variable(tf.truncated_normal([6*6*4*depth, num_hidden], stddev=0.1), name='W')
