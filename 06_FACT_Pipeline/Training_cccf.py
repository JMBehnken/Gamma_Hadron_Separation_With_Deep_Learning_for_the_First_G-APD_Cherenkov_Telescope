...
for num_steps, learning_rate, batch_size, patch_size, depth, num_hidden in hyperparameter:
    try:
        ...
        with tf.name_scope('fc_1'):
            layer4_weights = tf.Variable(tf.truncated_normal([6*6*4*depth, num_labels], stddev=0.1), name='W')
            layer4_biases = tf.Variable(tf.constant(1.0, shape=[num_labels]), name='B')
    
            output = tf.matmul(reshape, layer4_weights) + layer4_biases
        ...
        with tf.name_scope('validation'):
            ...
            valid_prediction = tf.nn.softmax(tf.matmul(reshape, layer4_weights) + layer4_biases)
