...
for num_steps, learning_rate, batch_size, patch_size, depth, num_hidden in hyperparameter:
    try:
        ...
        with tf.name_scope('fc_1'):
            layer6_weights = tf.Variable(tf.truncated_normal([2*2*16*depth, num_labels], stddev=0.1), name='W')
            layer6_biases = tf.Variable(tf.constant(1.0, shape=[num_labels]), name='B')
    
            output = tf.matmul(reshape, layer6_weights) + layer6_biases
        ...
        with tf.name_scope('validation'):
            ...
            valid_prediction = tf.nn.softmax(tf.matmul(reshape, layer6_weights) + layer6_biases)
